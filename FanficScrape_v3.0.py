...
def main():
	...
	#------ INIT ------
	# Window
	window = gui.Window(title='Fanfic Scrape', width=700, height=450, minwidth=500, minheight=350)
	...
	textframe.pack(side='bottom', fill='x', pady=20, padx=20)

	feedback = tk.Text(textframe, height=4, bg='#708090', fg='turquoise', wrap='word')
	update_text(feedback, 'ready')
	feedback.pack(expand='yes', fill='x')

	# Fetch content
	fetch = tk.Frame(out_frame)
	fetch.pack(anchor='s', expand='yes', fill='x', padx=20)
